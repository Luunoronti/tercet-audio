...
Kopia riaa/symlib.py (ten sam silnik parsowania .kicad_sym), rozni sie tylko
lokalizacja LIBDIR: riaa/symlib.py ma na stale linuksowa sciezke, tutaj
szukamy katalogu bibliotek KiCad po liscie kandydatow (Windows/Linux) +
zmienna srodowiskowa KICAD_SYMBOL_DIR (najwyzszy priorytet).
"""
import re
import uuid
import os
...
_CANDIDATES = [
    ...
    r"C:\Users\wikto\AppData\Local\Programs\KiCad\10.0\share\kicad\symbols",
...
def _find_libdir():
    for c in _CANDIDATES:
        if c and os.path.isdir(c):
            return c
    raise RuntimeError(
        "Nie znaleziono katalogu bibliotek symboli KiCad - "
        "ustaw zmienna srodowiskowa KICAD_SYMBOL_DIR na "
        "'.../share/kicad/symbols'"
    )
...
def load_lib(libname):
    ...
    with open(os.path.join(LIBDIR, libname + ".kicad_sym"), encoding='utf-8') as f:
        text = f.read()
